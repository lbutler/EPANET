...
- Support has been added for FAVAD (Fixed And Variable Area Discharge) modeling of pipe leaks:
 ...
 - Junctions that closed links cut off from every tank and reservoir are now taken out of service instead of receiving phantom flow. Previously such a junction stayed in the hydraulic equations coupled only through a closed link's small conductance placeholder, so its demand would "leak" through closed links at absurd heads (often reported as pressures in the millions), with most of the demand silently unserved - or, in rare cases, the run would fail with error 110. Now each isolated group of junctions - detected before the equations are solved at the start of every time step, and re-detected if mid-trial status changes leave junctions cut off where the solver would otherwise finish or fail - has all its heads fixed at the group's lowest junction elevation (a full but depressurized body of water at rest; the convention lives in one function, `islandhead()`), its demands, emitter flows and internal link flows zeroed and reported as undelivered, and the run finishes with warning 3 ("System disconnected") while the Status Report lists the disconnected junctions and the link that isolates them. An active PRV or FCV still passes flow for this purpose (its downstream section is supplied), while an active PSV does not (it head-fixes only its upstream node). `EN_DISCONNECTEDNODES` retrieves the number of junctions taken out of service. Results for networks that never have disconnected junctions are unchanged.
