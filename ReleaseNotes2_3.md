...
- Support has been added for FAVAD (Fixed And Variable Area Discharge) modeling of pipe leaks:
 ...
 - When a hydraulic analysis fails with error 110 the index of the node whose equation the solver could not factor can be retrieved with `EN_getstatistic` using `EN_ILLCONDITIONEDNODE`. The Status Report has always named that node, so this retrieves the index directly rather than requiring that line to be parsed. It is zero when the most recent analysis did not fail that way. Where a whole group of nodes is at fault the value names one member of it, since which equation the factorization stops on depends on how the equations were ordered.

 - A new `ISOLATION` option takes junctions that have no path to a supply source out of service instead of leaving them to receive phantom flow. It is off by default, so results are unchanged unless it is asked for, and can be set with `ISOLATION YES` in the `[OPTIONS]` section of an input file or with `EN_ISOLATION` through `EN_getoption` and `EN_setoption`. Previously such a junction stayed in the hydraulic equations coupled only through a closed link's small conductance placeholder, so its demand would "leak" through closed links at absurd heads (often reported as pressures in the millions), with most of the demand silently unserved - or, in rare cases, the run would fail with error 110. With the option in effect, each isolated group of junctions - detected before the equations are solved at the start of every time step, and re-detected if mid-step status changes leave junctions cut off where the solver would otherwise finish or fail - has all its heads fixed at the group's lowest junction elevation (a full but depressurized body of water at rest; the convention lives in one function, `islandhead()`), and its demands, emitter flows and internal link flows zeroed. The run finishes with warning 3 ("System disconnected") while the Status Report lists the disconnected junctions and the link that isolates them. A junction with a net inflow supplies the area around it and so counts as a source, as do the tanks and reservoirs and the node whose head an active PRV or PSV fixes. One-way links (check valves, PRVs, PSVs) are respected through the status logic that enforces them: a link that flows the wrong way is closed by a status check, and the detection - refreshed whenever the solver would otherwise finish or fail - then sees it as impassable. `EN_DISCONNECTEDNODES` retrieves the number of junctions taken out of service and `EN_ISOLATED` can be used with `EN_getnodevalue` to ask whether a particular junction is one of them.

 - Demand that is lost because a junction was taken out of service is now reported the way demand lost to low pressure under PDA already was: it is counted by `EN_DEFICIENTNODES` and `EN_DEMANDREDUCTION`, appears in the Status Report's reduced demand line, and is included in the Flow Balance Report's Demand Deficit, which now reports a shortfall under demand driven analysis as well as under pressure driven analysis. Each junction's own share is available from `EN_getnodevalue` with `EN_DEMANDDEFICIT`.
