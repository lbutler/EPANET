...
- Support has been added for FAVAD (Fixed And Variable Area Discharge) modeling of pipe leaks:
 ...
 - Junctions that closed links cut off from every tank and reservoir are now taken out of service instead of receiving phantom flow. Previously such a junction stayed in the hydraulic equations coupled only through a closed link's small conductance placeholder, so its demand would "leak" through closed links at absurd heads (often reported as pressures in the millions), with most of the demand silently unserved - or, in rare cases, the run would fail with error 110. Now, when a solution would otherwise end with demand-carrying junctions disconnected from all fixed grade nodes (or when the equation matrix becomes singular because of them), each disconnected group has its heads fixed at the group's lowest junction elevation and its demands are reported as undelivered, the run finishes with warning 3 ("System disconnected"), and the Status Report lists the disconnected junctions and the link that isolates them. `EN_DISCONNECTEDNODES` retrieves the number of junctions taken out of service. Results for networks without disconnected demand are unchanged.
